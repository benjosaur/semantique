...
def walk(path):
    ...
    start/⏎ are structural: they append nothing (but each hop costs 1).
    """
    pos = tuple(LEVEL["start"])
    words = []
    for i, target_word in enumerate(path):
        nxt = pos_of(target_word) if target_word not in ("start",) else tuple(LEVEL["start"])
...
def test_grid_shape_and_special_tiles():
    ...
    assert pos_of("start") == tuple(LEVEL["start"])
    assert LEVEL["targets"] and all(t in LEVEL["labels"] for t in LEVEL["targets"])


def test_quick_win_path():
    assert assemble_sentence(walk(["great", "!", "⏎"])) == "great!"


def test_clever_win_path():
    assert assemble_sentence(walk(["not", "sad", "⏎"])) == "not sad"


def test_start_revisit_win_path():
    assert assemble_sentence(walk(["very", "start", "great", "!", "⏎"])) == "very great!"


def test_min_hops_to_end_prevents_empty_submit():
    br, bc = pos_of("start")
    er, ec = pos_of("⏎")
    assert abs(br - er) + abs(bc - ec) >= 2  # ⏎ never adjacent to start
