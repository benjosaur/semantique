...
A level is self-contained: the grid geometry the browser draws **and** the
judge spec (which category to score, the few-shot pairs that pin the answer
format, the candidate labels). Adding a board is one file under levels/ that
defines `LEVEL = Level(...)` — see levels/__init__.py for the auto-discovery.
...
# Default question template. {category} names the thing to identify and the
# few-shot pairs teach the one-lowercase-word answer format.
DEFAULT_QUESTION = (
    'What {category} does this sentence describe, in one lowercase word: "{sentence}"'
)
...
class Level:
    ...
    category: str  # what the judge identifies, e.g. "emotion" / "animal"
    few_shot: tuple[tuple[str, str], ...] = ()  # (example sentence, answer) pairs
    question: str = DEFAULT_QUESTION  # template over {category} and {sentence}
    order: int = 100  # play order across boards (lower comes first)
    home: bool = False  # the board the app boots into

    def question_for(self, sentence: str) -> str:
        """Fill the question template for one example or the live sentence."""
        return self.question.format(category=self.category, sentence=sentence)
